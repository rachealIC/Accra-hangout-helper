...
interface RateLimitScreenProps {
  timeLeft: number;
  onSubscribe: (plan: 'micro-boost' | 'power-planner') => void;
}

const RateLimitScreen: React.FC<RateLimitScreenProps> = ({ timeLeft, onSubscribe }) => {
  return (
    <div className="flex flex-col justify-center items-center text-center p-4 h-full z-10 animate-slide-in">
      <div className="w-full max-w-3xl bg-white/60 dark:bg-slate-800/60 backdrop-blur-sm p-8 rounded-lg shadow-lg border border-white/50 dark:border-slate-700/50">
        ...
        <p className="text-[#660B05] dark:text-slate-300 mb-6 text-lg">Or choose a plan to continue planning now:</p>
        ...
        <div className="flex flex-col md:flex-row justify-center gap-6">
          ...
          <div className="flex-1 p-6 bg-white/80 dark:bg-slate-800 rounded-xl shadow-lg border border-gray-200 dark:border-slate-700 text-center flex flex-col">
            ...
            <button onClick={() => onSubscribe('micro-boost')} className="w-full mt-auto py-2 px-4 bg-[#8C1007] dark:bg-[#E18C44] text-white dark:text-slate-900 font-bold rounded-lg shadow-md hover:bg-[#660B05] dark:hover:bg-[#f3a469] transition-all">Pay GH₵3.00 via MoMo</button>
          </div>
          ...
          <div className="flex-1 p-6 bg-white/80 dark:bg-slate-800 rounded-xl shadow-lg border-2 border-[#8C1007] dark:border-[#E18C44] flex flex-col">
            ...
            <button onClick={() => onSubscribe('power-planner')} className="w-full mt-auto py-2 px-4 bg-[#8C1007] dark:bg-[#E18C44] text-white dark:text-slate-900 font-bold rounded-lg shadow-md hover:bg-[#660B05] dark:hover:bg-[#f3a469] transition-all">Subscribe for GH₵60.00/Month</button>
          </div>
        </div>
      </div>
    </div>
